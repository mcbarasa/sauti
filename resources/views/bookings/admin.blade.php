  ...
  <div class="table-scroll-wrap">
    <table class="admin-table">
      ...
      <tbody>
        ...
        @php
          $rowAccent = match($booking->status) {
            'confirmed' => 'rgba(52,199,123,0.04)',
            'pending'   => 'rgba(245,197,24,0.04)',
            'cancelled' => 'rgba(229,90,90,0.04)',
            'lapsed'    => 'rgba(100,100,100,0.04)',
            default     => 'transparent',
          };
          $borderLeft = match($booking->status) {
            'confirmed' => '#34C77B',
            'pending'   => '#F5C518',
            'cancelled' => '#E55A5A',
            'lapsed'    => '#555',
        ...
        @endphp
        <tr style="background:{{ $rowAccent }};border-left:3px solid {{ $borderLeft }};">
          <td>
            <span style="font-family:'DM Mono',monospace;font-size:0.75rem;
                         color:var(--gray);background:var(--surface3);
          ...
          <td>
            <div style="font-weight:700;color:var(--text);font-size:0.88rem;
                        display:flex;align-items:center;gap:0.4rem;flex-wrap:wrap;">
              {{ $booking->name }}
              @if($booking->is_recurring)
                <span style="background:rgba(90,158,229,0.15);color:#5A9EE5;
                             border-radius:20px;padding:1px 7px;font-size:0.6rem;
          ...
          <td>
            <a href="tel:{{ $booking->phone }}"
               style="font-family:'DM Mono',monospace;font-size:0.82rem;
          ...
          <td>
            ...
            @if($booking->overtime_amount > 0)
              <div style="font-size:0.68rem;color:#E55A5A;margin-top:2px;">
                +KES {{ number_format($booking->overtime_amount) }} OT
              </div>
            @endif
          </td>

          <td class="col-notes"
              style="font-size:0.8rem;max-width:140px;white-space:nowrap;
                     overflow:hidden;text-overflow:ellipsis;color:var(--gray);"
              title="{{ $booking->notes }}">
            {{ $booking->notes ?: '—' }}
          </td>
              ...
              <form action="{{ route('bookings.confirm', $booking) }}" method="POST"
                ...
                <button type="submit"
                  style="display:inline-flex;align-items:center;gap:4px;
                         ...
                         background:rgba(52,199,123,0.15);color:#34C77B;
                         font-family:'DM Sans',sans-serif;transition:all 0.15s;"
                  onmouseover="this.style.background='rgba(52,199,123,0.3)'"
                  onmouseout="this.style.background='rgba(52,199,123,0.15)'">
                  ✓ Confirm
                </button>
              </form>
              @endif
...
<form action="{{ route('bookings.destroy', $booking) }}" method="POST"
      style="display:inline;"
      onsubmit="return showCancelConfirm(event, this, '{{ $booking->id }}', '{{ addslashes($booking->name) }}')">
  @csrf @method('DELETE')
...
@endif
              ...
              {{-- Delete (hard delete) --}}
<form action="{{ route('bookings.delete', $booking) }}" method="POST"
      style="display:inline;"
      onsubmit="return showDeleteConfirm(event, this, '{{ $booking->id }}', '{{ addslashes($booking->name) }}')">
  @csrf @method('DELETE')
  <button type="submit"
    style="display:inline-flex;align-items:center;gap:4px;
           padding:0.3rem 0.75rem;border-radius:4px;font-size:0.72rem;
           font-weight:600;border:1px solid rgba(229,90,90,0.3);
           cursor:pointer;background:rgba(229,90,90,0.1);
           color:#E55A5A;font-family:'DM Sans',sans-serif;
           transition:all 0.15s;"
    onmouseover="this.style.background='rgba(229,90,90,0.25)'"
    onmouseout="this.style.background='rgba(229,90,90,0.1)'">
    🗑 Delete
  </button>
</form>
...
async function checkoutSession(bookingId, btn) {
  ...
  try {
    ...
    if (data.success) {
      applyLapsedToCard(bookingId, data);   // ← no btn argument
      showCheckoutResult(data);

    } else if (data.requires_payment) {
      btn.disabled    = false;
      btn.textContent = '🔴 Check Out (Overtime)';
      showOvertimePaymentPending(data, bookingId, t);
...
function showOvertimePaymentPending(data, bookingId, t) {
  ...
  modal.innerHTML = `
    <div style="background:var(--surface);border:1px solid rgba(229,90,90,0.4);
      ...
      <p style="color:var(--gray);font-size:0.85rem;margin-bottom:1.25rem;">
        M-Pesa prompt sent to <strong style="color:var(--text);">${data.phone}</strong>
      </p>
      ...
      <div style="display:flex;gap:0.75rem;">
        ...
        <button id="ot-manual-btn"
          style="flex:2;padding:0.65rem;border-radius:4px;
                 border:1px solid rgba(245,197,24,0.4);background:rgba(245,197,24,0.1);
  ...
  // ── Finalize: called by poll AND manual ───────────────────────────
  function finalizeCheckout() {
    ...
    // applyLapsedToCard adds to _lapsedLocally, updates sessionData,
    // freezes the card — tickAll and fetchSessions will both respect it
    applyLapsedToCard(bookingId, resultData);
...
function showCancelConfirm(e, form, bookingId, name) {
  e.preventDefault();
  ...
  modal.innerHTML = `
    <div style="background:var(--surface);border:1px solid rgba(245,197,24,0.35);
                border-radius:10px;padding:2rem;max-width:400px;width:100%;
                text-align:center;">
      <div style="font-size:2.5rem;margin-bottom:0.75rem;">✕</div>

      <h3 style="font-family:'Bebas Neue',sans-serif;font-size:1.6rem;
                 letter-spacing:2px;color:var(--yellow);margin-bottom:0.5rem;">
        Cancel Booking
      </h3>

      <p style="color:var(--text);font-size:0.95rem;margin-bottom:0.35rem;">
        <strong>#${String(bookingId).padStart(3,'0')} — ${name}</strong>
      </p>

      <div style="background:rgba(245,197,24,0.07);border:1px solid rgba(245,197,24,0.2);
                  border-radius:6px;padding:0.75rem 1rem;margin:1rem 0 1.25rem;">
        <p style="color:var(--yellow);font-size:0.82rem;margin:0;">
          The booking will be marked as cancelled. This can be reversed by confirming again.
        </p>
      </div>

      <div style="display:flex;gap:0.75rem;">
        <button id="cxl-cancel-btn"
          style="flex:1;padding:0.7rem;border-radius:4px;
                 border:1px solid var(--border);background:transparent;
                 color:var(--text2);font-family:'DM Sans',sans-serif;
                 font-size:0.88rem;font-weight:600;cursor:pointer;">
          Go Back
        </button>
        <button id="cxl-confirm-btn"
          style="flex:2;padding:0.7rem;border-radius:4px;border:none;
  ...
  document.getElementById('cxl-cancel-btn').addEventListener('click', () => {
    modal.remove();
  });

  document.getElementById('cxl-confirm-btn').addEventListener('click', () => {
    modal.remove();
    form.submit();
  });
...
function showDeleteConfirm(e, form, bookingId, name) {
  e.preventDefault();
  ...
  modal.innerHTML = `
    <div style="background:var(--surface);border:1px solid rgba(229,90,90,0.4);
      ...
      <div style="background:rgba(229,90,90,0.08);border:1px solid rgba(229,90,90,0.2);
                  border-radius:6px;padding:0.75rem 1rem;margin:1rem 0 1.25rem;">
        <p style="color:#E55A5A;font-size:0.82rem;margin:0;">
          ⚠ This will permanently remove this booking and cannot be undone.
        </p>
      </div>
      ...
      <div style="display:flex;gap:0.75rem;">
        ...
        <button id="del-confirm-btn"
          style="flex:2;padding:0.7rem;border-radius:4px;border:none;
                 background:#E55A5A;color:#fff;font-family:'DM Sans',sans-serif;
                 font-size:0.88rem;font-weight:700;cursor:pointer;letter-spacing:0.5px;">
